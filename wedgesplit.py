...
def plot_mm_vs_kn(mm, kn, title):
    ...
    plt.annotate(f'Max Value: {max_Force:.2f}', xy=(max_mm, max_Force), xytext=(max_mm, max_Force-1),
             arrowprops=dict(facecolor='#7FCDBB', shrink=0.2, width = 0.5, headwidth = 4, edgecolor ='#7FCDBB'), fontsize = 12)
...
def plot_mm_vs_Fs(mm, kn, title):
    ...
    plt.minorticks_on()
    plt.legend(fontsize=12)
    plt.annotate(f'Max Value: {max_Force:.2f}', xy=(max_mm, max_Force), xytext=(max_mm, max_Force-0.5),
